...
const PrisonList = (props: any) => {
  ...
  const handleViewById=(id:string)=>{
    history.push(`/sysAdmin/prison/${id}`);
  }
  return (
        ...
        <Table stickyHeader aria-label="sticky table">
          ...
          <TableBody>
            {prisons
              ...
              .map((prison) => {
                return (
                  <TableRow hover role="checkbox" tabIndex={-1} key={prison.id}>
                
                      
                        <TableCell >
                        <Link onClick={()=>handleViewById(prison.id)}>{prison.names}</Link>
                        </TableCell>
                        <TableCell >
                         {prison.location}
                        </TableCell>
